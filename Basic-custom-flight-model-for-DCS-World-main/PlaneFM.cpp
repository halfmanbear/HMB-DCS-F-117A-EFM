...
void ed_fm_simulate(double dt)
{
	...
	*/
	...
	PlaneFM::aileron_DEG = limit(PlaneFM::aileron_DEG, (-15.0 * roll_filter), (15.0 * roll_filter));

	//Dragshute
	PlaneFM::dragshute = PlaneFM::ACTUATORS::dragshute_actuator(PlaneFM::dragshute_command, dt);
	...
	PlaneFM::rudder_DEG = limit(PlaneFM::rudder_DEG, -15.0 / aos_filter, 15.0 / aos_filter);
		...
		PlaneFM::misc_stateH = PlaneFM::ACTUATORS::misc_actuatorH(PlaneFM::misc_cmdH, dt); //hook
		...
		// Drag Shute aero
		double CDshute = 1.0 * PlaneFM::ACTUATORS::dragshute_state; //Drag influence
		double Cxshute = -(CDshute * cos(PlaneFM::pi / 180.0));
	...
	// Cx is drag
		...
		PlaneFM::AERO::Cx_total += CxGear + Cxbrakes;
...
//		break;
	...
	case DragShute: //toggle
		if (PlaneFM::ACTUATORS::dragshute_state < 0.25)
			PlaneFM::dragshute_command = 1.0;
		else if (PlaneFM::ACTUATORS::dragshute_state > 0.75)
			PlaneFM::dragshute_command = 0.0;
		printf("Drag Shute = %f \n", dragshute_command);
		break;
	...
	case WheelBrakeOff:
		...
		//Other commands
	case tailhook:  
		if (misc_stateH < 0.5) PlaneFM::misc_cmdH = 1.0;
		if (misc_stateH > 0.5) PlaneFM::misc_cmdH = 0.0;
		break;
...
void ed_fm_set_draw_args(EdDrawArgument* drawargs, size_t size) //The things that move on the model, use the model viewer to learn what each "arg" corresponds to.
{
	...
	// Drag Shute
	drawargs[35].f = (float)ACTUATORS::dragshute_state;

	// Weapon bay doors or tail hook
	drawargs[25].f = (float)limit((PlaneFM::misc_stateH), 0.0, 1.0); // This is usually the tail hook for most planes with them.
	drawargs[900].f = (float)limit((PlaneFM::misc_state), 0.0, 1.0); // This is usually weapon bays for planes with them.
...
{
	drawargs[1001] = (float)limit((PlaneFM::FLIGHTCONTROLS::latStickInput), -1.0, 1.0);
	drawargs[1000] = (float)limit((-PlaneFM::FLIGHTCONTROLS::longStickInput), -1.0, 1.0);
	drawargs[1002] = (float)limit((PlaneFM::throttleInput), -1.0, 1.0);
	drawargs[1003] = (float)limit((PlaneFM::pedInput), -1.0, 1.0);
	//drawargs[105] = (float)limit((PlaneFM::throttleInput), 0.0, 1.0);

	//PlaneFM::FLIGHTCONTROLS::latStickInput = drawargs[1001];
	//PlaneFM::FLIGHTCONTROLS::longStickInput = drawargs[1000];
	//PlaneFM::throttleInput = drawargs[1002];
...
void ed_fm_release()
{
	...
	PlaneFM::dragshute_command = 0.0;
	PlaneFM::dragshute = 0.0;
	PlaneFM::flap_command = 0.0;
	PlaneFM::flaps = 0.0;
	PlaneFM::misc_cmd = 0.0;
	PlaneFM::misc_state = 0.0;
	PlaneFM::misc_cmdH = 0.0;
	PlaneFM::misc_stateH = 0.0;
