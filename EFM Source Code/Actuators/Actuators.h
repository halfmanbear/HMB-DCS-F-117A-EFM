...
namespace F117
{
	namespace ACTUATORS
	{
		...
		double  dragshute_actuator(double dragshute_command, double frameTime)
		{
			if (!simInitialized)
			{
				dragshute_state = dragshute_command;
				return dragshute_state;
			}
			dragshute_rate = 20.2 * (dragshute_command - dragshute_state);
			dragshute_rate = limit(dragshute_rate, -0.75, 0.75);
			dragshute_state += (dragshute_rate * frameTime);
			dragshute_state = limit(dragshute_state, 0.0, 1.0);
		...
		float  misc_actuatorH(float misc_cmdH, double frameTime)
		{
			...
			misc_rateH = 20.2 * (misc_cmdH - misc_posH);
			misc_rateH = limit(misc_rateH, -0.5, 0.5);
			misc_posH += (misc_rateH * frameTime);
			misc_posH = limit(misc_posH, 0.0, 1.0);
